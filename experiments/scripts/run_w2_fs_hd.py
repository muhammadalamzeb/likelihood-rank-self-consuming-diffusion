#!/usr/bin/env python3
"""
W2-1 HD transfer: imbalanced GMM in D>2 with tiny MLP DDPM.
Experiment ID: w2_fs_hd
"""
...
def make_gmm_hd(n: int, K: int, dim: int, delta: float, rho: float, seed: int):
    rng = np.random.default_rng(seed)
    # Orthonormal-ish directions via QR
    A = rng.normal(size=(dim, K)).astype(np.float64)
    Q, _ = np.linalg.qr(A)
    means = (delta * Q[:, :K].T).astype(np.float32)  # [K, dim]
    ...
    for k, c in enumerate(counts):
        if c == 0:
            continue
        parts.append(means[k] + 0.15 * rng.normal(size=(c, dim)).astype(np.float32))
...
def main():
    ...
    ap.add_argument("--log-name", type=str, default="w2_fs_hd.jsonl")
    ...
    cfg["experiment"] = "w2_fs_hd"
    with open(out / "configs" / "w2_fs_hd.json", "w", encoding="utf-8") as f:
        json.dump(cfg, f, indent=2)

    all_recs = []
    for seed in args.seeds:
        rng = np.random.default_rng(seed)
        X0, _, means, true_w = make_gmm_hd(
            args.n_data, args.K, args.dim, args.delta, args.rho, seed
        )
        real_buf = X0.copy()
        X_hold, _, _, _ = make_gmm_hd(
            args.n_data, args.K, args.dim, args.delta, args.rho, seed + 999
        )
